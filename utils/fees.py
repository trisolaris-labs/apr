...
def convertFeesForPair(tri_maker, pair, w3, acct):
    ...
    try:
        ...
        convert_tranasction = tri_maker.functions.convert(pair[0], pair[1]).buildTransaction(
            transaction
        )
...
def convertFeesForPairs(tri_maker, pairs, w3, acct):
    ...
    try:
        ...
        convert_tranasction = tri_maker.functions.convertMultiple(token0, token1).buildTransaction(
            transaction
        )
        signed = w3.eth.account.sign_transaction(convert_tranasction, acct.key)
        signed_txn = w3.eth.sendRawTransaction(signed.rawTransaction)
...
def convertStablestoLP(stable_lp_maker, w3, acct):
    ...
    try:
        ...
        convert_tranasction = stable_lp_maker.functions.convertStables().buildTransaction(
            transaction
        )
        signed = w3.eth.account.sign_transaction(convert_tranasction, acct.key)
        signed_txn = w3.eth.sendRawTransaction(signed.rawTransaction)
